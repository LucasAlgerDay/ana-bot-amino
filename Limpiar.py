...
client = BotAmino(email="gakarok751@asimark.com", password="spam123")
client.prefix = "!"
...
def inviteall(data):
    if client.check(data,'staff'):
        ...
        for i in range(1):
            data.subClient.send_message(chatId=data.chatId,message=f"[Ic]⸻ ❝Initado a {u} usuarios , {data.author}❞ ৴", replyTo=data.messageId)
...
[C]𐄁𐄙𐄁𐄙𐄁𐄙𐄁𐄙𐄁𐄙𐄁𐄙𐄁𐄙𐄁
""",embedTitle="❝ 🧸̱﹕𝗝꯭𝗢꯭𝗡 ⸴⸴",embedLink="http://aminoapps.com/u/OtracuentamasdeJonaqui",embedImage=im,embedContent=" C R E A D O R  D E  A N A")
...
def ship(data):
			...
			for user in mention:
				...
				msg[1] = msg[0]
				try:
					data.subClient.send_message(chatId=data.chatId,file=imgg,fileType="image",embedTitle=f"ℓσѵε ɱαƭcɦ ❴ {random.randint(0,100)}% ❵",embedLink="http://aminoapps.com/u/Jon530868",embedContent="𝑀𝑎𝑑𝑒 𝑏𝑦 ⁶❝ 🧸̱﹕𝗝꯭𝗢꯭𝗡 ⸴⸴⁹",embedImage=img9)
				except:
					pass
...
def love(data):
			...
			for user in mention:
				...
				msg[1] = msg[0]
				try:
					data.subClient.send_message(chatId=data.chatId,file=imgg,fileType="image",embedTitle=f"Love match❤️ {random.randint(0,100)}% ❵",embedLink="http://aminoapps.com/u/Jon530868",embedContent="𝑀𝑎𝑑𝑒 𝑏𝑦 ❝ 🧸̱﹕𝗝꯭𝗢꯭𝗡 ⸴⸴』⁹",embedImage=img9)
				except:
					pass
...
def hate(data):
			...
			for user in mention:
				...
				msg[1] = msg[0]
				try:
					data.subClient.send_message(chatId=data.chatId,file=imgg,fileType="image",embedTitle=f"Hate💔 ❴ {random.randint(0,100)}% ❵",embedLink="http://aminoapps.com/u/Jon530868",embedContent="𝑀𝑎𝑑𝑒 𝑏𝑦 ❝ 🧸̱﹕𝗝꯭𝗢꯭𝗡 ⸴⸴",embedImage=img9)
				except:
					pass
...
def background(data):
	...
	if reply_message:
		...
		for i in range(1,3):
			try:
				data.subClient.edit_profile(backgroundImage=image)
			except Exception:
...
def bgcolor(data):
    if client.check(data,'staff'):
        ...
        data.subClient.send_message(chatId=data.chatId,message="[Ic]⸻Listo, fondo de color puesto, {data.author}❞ ৴",replyTo=data.messageId)
